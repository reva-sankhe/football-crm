...
try {
  ...
  const {
    buildSquadWeeklyLoad, computeSquadUsualLoadRange, buildLoadToWatch,
    buildDataQualityPanel, interpretSquadWeeklyLoad, interpretLoadToWatch,
  ...
  const weeklyRows = [
    row("p1", "2026-08-04", 200), row("p2", "2026-08-06", 600), // week 1: 2 players, 800 total
    // week 2 (08-10..16): nothing at all — a genuine team gap
    row("p1", "2026-08-20", 500), // week 3: 1 player
  ];
  const weekly = buildSquadWeeklyLoad(weeklyRows, start, end);
  ...
  assert.equal(weekly[1].perPlayerAu, 0, "zero players is a real zero average, not a division-by-zero artifact");
  assert.equal(weekly.every((w) => !w.isPartial), true, "an exact multiple of 7 days has no partial week");

  // perPlayerAu divides by turnout (players who actually logged), not a constant squad size.
  assert.equal(weekly[0].totalAu, 800);
  assert.equal(weekly[0].players, 2);
  approxEqual(weekly[0].perPlayerAu, 800 / 2, "divided by the 2 players who actually logged this week");
  assert.equal(weekly[2].players, 1);
  approxEqual(weekly[2].perPlayerAu, 500, "a single-player week: per-player average is just that player's own total");

  // Week-on-week: null for the first week; a real drop into the zero week; null out of it (previous was 0).
  ...
  const partial = buildSquadWeeklyLoad(weeklyRows, start, partialEnd);
  ...
  const estimatedWeekly = buildSquadWeeklyLoad(estimatedRows, new Date(2026, 7, 3), new Date(2026, 7, 9));
  ...
  // Build 9 clean weeks (8 history + 1 current), one constant-turnout player, for a clean band.
  const nineWeeksRows = [];
  for (let i = 0; i < 9; i++) {
    nineWeeksRows.push(row("p1", new Date(2026, 5, 1 + i * 7).toISOString().slice(0, 10), 100));
  }
  const nineWeekStart = new Date(2026, 5, 1);
  const nineWeekEnd = new Date(2026, 5, 1 + 8 * 7 + 6); // 9 full weeks
  const nineWeekly = buildSquadWeeklyLoad(nineWeeksRows, nineWeekStart, nineWeekEnd);
  ...
  const tenWeeksRows = [row("p1", "2026-01-01", 999999), ...nineWeeksRows];
  // Shift everything so the huge outlier falls outside the most-recent-12 window relative to the same final week.
  const withOutlier = buildSquadWeeklyLoad(tenWeeksRows, new Date(2026, 0, 1), nineWeekEnd);
  ...
  // acute window is [2026-08-24, 2026-08-30]. Fixture rows are placed inside
  // that real window (plus one row before baselineStart, so firstLogged <=
  // baselineStart and hasBaseline is actually true).
  const players3 = [player("p1", "Spiky"), player("p3", "Steady")];
  ...
  assert.ok(!watchNames.includes("Steady"), "a perfectly flat player is Typical, not on the watch list — nobody but Spike/Elevated ever appears");
  ...
  assert.equal(interpretLoadToWatch([]), "Nobody is above their usual load range right now.");
  assert.match(interpretLoadToWatch(watch), /above their usual range/);
